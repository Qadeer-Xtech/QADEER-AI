...
/**
 * Command to download an audio (MP3) from YouTube.
 * It can take a URL or a search query.
 */
cmd({
    pattern: "song2",
    alias: ["audio2", "ytaudio2"],
    react: "🎶",
    desc: "Download audio from YouTube using savetube.",
    category: "download",
    ...
    try {
        ...
        if (!result.status) {
            return await reply(`❌ Failed to download audio. Error: ${result.error}`);
        }

        const audioInfo = result.response;
        
        if (!audioInfo || !audioInfo.download) {
            return await reply("❌ Could not retrieve the download link.");
        }

        await conn.sendMessage(from, {
            audio: { url: audioInfo.download },
            mimetype: 'audio/mpeg',
