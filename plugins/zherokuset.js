...
ezra({
  nomCom: 'anticall',
  categorie: "QADEER-SETTING"
}, async (chatId, zk, context) => {
  const { ms, repondre, superUser, auteurMessage, arg } = context;

  // Check if the command is issued by the owner
  if (!superUser) {
    return repondre("*This command is restricted to the bot owner. or QADEER AI owner* 🤦,,idiot");
  }
...
ezra({
  ...
  // Check if the command is issued by the owner
  if (!superUser) {
    return repondre("*This command is restricted to the bot owner or QADEER owner.* 🤦,,idiot");
  }
