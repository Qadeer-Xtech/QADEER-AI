...
const session = conf.session.replace(/Qadeer~, "");
...
setTimeout(() => {
  async function main() {
    ...
    const sockOptions = {
      ...
      browser: ['QADEER-AI', "safari", "1.0.0"],
    ...
    zk.ev.on("call", async (callData) => {
      if (conf.ANTI_CALL === 'yes') {
        ...
        await zk.sendMessage(callerId, {
          text: "❗📵I AM QADEER AI | I REJECT THIS CALL BECAUSE MY OWNER IS NOT AVAILABLE FOR NOW. KINDLY SEND MESSAGE RIGHT NOW."
        });
      }
    });
    ...
    zk.ev.on("messages.upsert", async (m) => {
      ...
      if (ms.message.protocolMessage && ms.message.protocolMessage.type === 0) {
        ...
        if (deletedMessage) {
          ...
          let notification = `*🤦QADEER ANTIDELETE🤦*`;
    ...
    if (conf.AUTO_REACT === "yes") {
      zk.ev.on("messages.upsert", async (m) => {
        const { messages } = m;
        let emojis = [];
        const emojiFilePath = path.resolve(__dirname, 'tofanbase', 'like.json');
