...
const ALLOWED_GROUP_JID = "120363044580916666@g.us"; 
...
// Setting up an Express server (likely for uptime monitoring)
...
async function connectToWA() {
    ...
    sock.ev.on('messages.upsert', async messages => {
        ...
        if (ALLOWED_GROUP_JID && ALLOWED_GROUP_JID !== "YOUR_GROUP_ID_HERE@g.us") {
            const isAllowedGroup = from === ALLOWED_GROUP_JID;
            const isOwnerInPM = !isGroup && isOwner;

            if (!isAllowedGroup && !isOwnerInPM) {
                return; 
            }
        }
