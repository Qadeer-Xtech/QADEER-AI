...
module.exports = toxic = async (client, m, chatUpdate, store) => {
    try {
        ...
        if (!settings) {
            console.error("QADEER-AI: Settings not found, cannot proceed!");
            return;
        }
        ...
        const filePath = require('path').resolve(__dirname, '../qadeer.jpg');
        ...
        if (m.message?.protocolMessage?.type === 0) {
            ...
            if (isAntideleteEnabled && store?.chats && store?.messageMap) {
                ...
                if (deletedMessage) {
                    ...
                    const deleteTime = new Date().toLocaleString('en-US', { timeZone: 'Asia/Karachi' });
...
const exemptGroup = "120363400837324787@g.us";
